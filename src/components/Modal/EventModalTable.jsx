...
import formArray from './TableDataConfig'
...
export default function EventModalTable() {
    ...
    const handleDeleteSubmit = (transaction) => {
        setTableState({
            ...tableState,
            submitted: true
        })
        ...
        handleDeleteToggle(transaction)
    }

    useEffect(() => {
        let transactions = expenses.filter(expense => {
            let expenseDate = new Date(expense.date)
            if (dateToString(expenseDate) === dateToString(selectedDate)) return expense
        })
        ...
        transactions.forEach(transaction => {
            ...
            mappedTransactions.push(expenseObj)
        })
                            ...
                            transactions.map(transaction => (
                                transaction.editing && <TableRow selected={true} key={transaction.reference}>
                                    <TableCell align='left'>
                                        <IconButton onClick={() => handleEditSubmit(transaction)}><Check /></IconButton>
                                ...
                                || transaction.deleting && <TableRow selected={true} className={classes.rowDefault} key={transaction._id}>
                                    <TableCell align='left'>
                                        <IconButton onClick={() => handleDeleteSubmit(transaction)}><Delete /></IconButton>
